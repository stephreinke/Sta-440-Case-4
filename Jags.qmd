...
# --- Prepare data (assumes x2 is your data.frame with columns:
...
bdat <- list(
  ...
  N = nrow(x2),
  ID = x2$IDnum,
  n_t = length(unique(x2$IDnum)),
  X = x2$Midline_s,
  outer = x2$outer,
  outer_wt = x2$outer_wt
)

# initial values function
fun.inits <- function() {
  # rough method-of-moments starts:
  muA <- log(max(1, median(x2$minDB, na.rm=TRUE)/2))  # crude
  muD <- log(abs(diff(range(x2$minDB, na.rm=TRUE))/2))
  muEta <- qlogis(0.5)  # 0

...
# --- Prepare df (use your rootData) ---
df <- rootData %>%
  rename(Midline = Midline, Length = Length, side = side, Genotype = Genotype) %>%
  mutate(Genotype = factor(Genotype),
         side = factor(side),
         isMU = ifelse(Genotype == "MU", 1, 0),
         isOuter = ifelse(side == "Outer", 1, 0))

# required: no NA in Midline/Length/isMU/isOuter
stopifnot(!any(is.na(df$Midline)))
stopifnot(!any(is.na(df$Length)))
stopifnot(all(df$isMU %in% c(0,1)))
stopifnot(all(df$isOuter %in% c(0,1)))

# Response on log scale
df$logLength <- log(df$Length)

# --- Write JAGS model file (natural-scale parameters, log-response) ---
cat("
model {
  for (i in 1:N) {
...
data_list <- list(
  ...
  isMU     = df$isMU,
  isOuter  = df$isOuter,
  # pass the median of Midline so JAGS can center xmid prior sensibly
  median_Midline = median(df$Midline, na.rm = TRUE)
)

# --- Initial values (matching parameter names in model) ---
inits_fn <- function() {
  list(
    Lmin = runif(1, max(0.1, quantile(df$Length, 0.01)), quantile(df$Length, 0.25)),
    Lmax0 = runif(1, quantile(df$Length, 0.75), quantile(df$Length, 0.99)),
    Lmax_gen = rnorm(1, 0, 0.1),
    Lmax_side = rnorm(1, 0, 0.1),
    Lmax_int = rnorm(1, 0, 0.1),
    xmid = median(df$Midline, na.rm = TRUE),
    s = runif(1, (max(df$Midline)-min(df$Midline))/50, (max(df$Midline)-min(df$Midline))/4),
    sigma = runif(1, 0.1, sd(df$logLength))
  )
}

# --- Parameters to save ---
params <- c("Lmin","Lmax0","Lmax_gen","Lmax_side","Lmax_int","xmid","s","sigma",
            "Lmax_monitor","mu_nat_monitor")
...
jags_out <- jags(
  ...
  model.file = "rootModel_formlog.jags",
  n.chains = 4,
  n.iter = 25000,
  n.burnin = 6000,
  n.thin = 4,
...
total_draws <- n_chains * iters_per_chain

cat("MCMC Summary:\n")
cat("  Number of chains: ", n_chains, "\n")
cat("  Iterations per chain: ", iters_per_chain, "\n")
cat("  Total posterior draws: ", total_draws, "\n\n")

# --- Effective sample sizes ---
ess_vals <- effectiveSize(mcmc_chains)
cat("Effective Sample Sizes (ESS):\n")
print(summary(ess_vals))

# Show smallest and largest ESS for a quick check
cat("\nRange of ESS:\n")
cat("  Min ESS:", min(ess_vals), "\n")
cat("  Max ESS:", max(ess_vals), "\n\n")

# Optional: short summary table for top parameters
ess_table <- sort(ess_vals, decreasing = TRUE)
head(ess_table)
```

---
title: "Nonlinear Regression Models for Root Curvature - STA440 Case 4"
...
$$
\log(\text{Length}_{ij}) = L + \frac{U_{ij} - L}{1 + \exp\left(-\frac{M_{ij} - \text{xmid}_{ij}}{s_{ij}}\right)}
$$

Where: L: lower asymptote (minimum log Length), $U_{ij}$: upper asymptote (maximum log Length) for observation i in root j, $xmid_{ij}$: midpoint (inflection point) of sigmoid curve, $s_{ij}$: scale parameter, $M_{ij}$: scaled midline position
...
Fixed Effects (same as frequentist)
...
```{r tab:gelman-diagnostics, echo=FALSE}
# Table 2: Gelman-Rubin convergence diagnostics for main parameters
library(knitr)

gelman_results <- gelman.diag(sims, transform = TRUE, multivariate = FALSE)

kable(gelman_results$psrf,
      caption = "Gelman-Rubin convergence diagnostics for main parameters",
      digits = 3)
```

```{r tab:effective-size, echo=FALSE}

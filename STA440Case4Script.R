...
# predictions and posterior means
...
# predicted quantities per observation
...
p_mu_bayes <- ggplot(filter(rootData, Genotype == "MU"),
  ...
  facet_wrap(~ root, ncol = 3) +
  theme_bw() + 
  labs(title = "Bayesian Fit: MU", x = "Scaled Midline", y = "Cell Length")
print(p_mu_bayes)
...
# build predicted log-length per draw
...
# 4) Posterior predictive p-value
